...
export default function SupplierIncomingOrders({ user }) {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const data = await api.supplierOrders(user.userId).catch(() => []);
      setOrders(data || []);
    } catch (err) {
            ...
            <table className="data-table">
              ...
              <tbody>
                {pendingOrders.length > 0 ? pendingOrders.map(order => {
                  ...
                  <tr><td colSpan="5" style={{ textAlign: 'center', padding: 28, color: 'var(--text-muted)' }}>No new pending requests.</td></tr>
                )}
              </tbody>
            </table>
          </div>
